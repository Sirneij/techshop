...
from techshop.settings import PRODUCTS_PER_ROW
import secrets
...
def tracking_id(request):
    ...
    try:
        ...
        request.session['tracking_id'] = secrets.token_urlsafe(16)
        return request.session['tracking_id']
...
def get_recently_viewed(request):
    ...
    return Product.objects.filter(id__in=product_ids, active=True)
